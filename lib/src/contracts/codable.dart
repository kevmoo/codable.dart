...
/// Base contract for domain objects that can serialize themselves to an [Encoder].
abstract interface class Encodable {
  /// Encodes this instance into the given [encoder].
  void encode(Encoder encoder);
}

/// Base contract for domain objects that can deserialize themselves from a [Decoder].
abstract interface class Decodable<T> {
  /// Decodes and returns an instance of [T] from the given [decoder].
  T decode(Decoder decoder);
}

/// Unified bi-directional serialization contract combining [Encodable] and [Decodable].
abstract interface class Codable<T> implements Encodable, Decodable<T> {}
